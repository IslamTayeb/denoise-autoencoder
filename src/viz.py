...
def plot_histories_overlay(
    histories: dict[str, dict], out_path: Path | str, title: str = ""
) -> None:
    p = _ensure_parent(out_path)
...
def plot_residual_hist(
    ...
    ax.hist(
        observed, bins=edges, density=True, alpha=0.5, label="observed", color="black"
    )
...
def plot_qq(
    observed: np.ndarray, synthetic: np.ndarray, out_path: Path | str, title: str = ""
) -> None:
    p = _ensure_parent(out_path)
...
def plot_heatmap(
    ...
    title: str = "",
    cbar_label: str = "",
    xlabel: str = "eval noise",
    ylabel: str = "train noise",
) -> None:
    p = _ensure_parent(out_path)
...
def plot_real_eval(
    t, real, fitted, denoised, out_path: Path | str, title: str = ""
) -> None:
    p = _ensure_parent(out_path)
